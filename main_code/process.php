...
if (isset($_POST['login'])) {
    ...
    $sql = mysqli_query($conn, $query);
    header('location: admins.php');
} elseif (isset($_POST['id_barang']) && isset($_GET['edit'])) {
    ...
    if (!empty($_FILES['link_gambar']['name'])) {
        $target_dir = "uploads/";
        $target_file = $target_dir . basename($_FILES['link_gambar']['name']);
        $upload_ok = 1;


        $check = getimagesize($_FILES['link_gambar']['tmp_name']);
        if ($check === false) {
            echo "File yang diupload bukan gambar.";
            $upload_ok = 0;
        }

        if ($_FILES['link_gambar']['size'] > 2000000) {
    ...
    $sql = "UPDATE tb_barang SET
                ...
                harga_barang = ?,
                stok_barang = ?";
    $params = [$nama_barang, $harga_barang, $stok_barang];
    ...
    $sql = "INSERT INTO tb_barang (nama_barang, harga_barang, stok_barang, link_gambar) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);

    if ($stmt->execute([$nama_barang, $harga_barang, $stok_barang, $link_gambar])) {
        $_SESSION['success_message'] = "Barang berhasil ditambahkan.";
        header("Location: admins.php");
        exit;
    } else {

        $_SESSION['error_message'] = "Terjadi kesalahan saat menambahkan barang.";
        header("Location: admins.php");
        exit;
    }
}
