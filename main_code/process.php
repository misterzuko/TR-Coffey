...
if(isset($_FILES['link_gambar'])){
    ...
    header('location: admins.php?addberhasil');
        ...
        elseif (isset($_POST['edit'])){
            ...
            if(isset($_FILES['link_gambar'])){
                $source=$_FILES['link_gambar']['tmp_name'];
                $destination=$_SESSION['data-admin'][$_POST['edit']]['link_gambar'];
                if(!move_uploaded_file($source,$destination)){
                    $link="";
                } else $link=$destination;
            } else $link="";
            try{
                $_nama = $jenis." ".$nama;
                $query = "UPDATE tb_barang SET nama_barang='$_nama', harga_barang='$harga', stok_barang='$stok', link_gambar='$link' WHERE id_barang='$id'";
                $sql = mysqli_query($conn,$query);
                updatedataAdmin();
